...
public class RemoteSession {
	...
	public void sendKeys() throws Exception{
		// Initialising Key Objects
		KeyPairGenerator genPrivateKey = KeyPairGenerator.getInstance("RSA");
		KeyGenerator secretKeyGen = KeyGenerator.getInstance("HmacSHA256");

		// Initialising the Key
		genPrivateKey.initialize(2048);

		// Generating the Keys
		KeyPair keyPair = genPrivateKey.generateKeyPair();
		...
		// System.out.println("SECRET KEY STUFF");
		// System.out.println("Public key (B64):" + Base64.getEncoder().encodeToString(publicKeyBytes));
		// System.out.println("Secret key (B64):" + Base64.getEncoder().encodeToString(secretKeyBytes));

		writer.println(Base64.getEncoder().encodeToString(publicKeyBytes));
		writer.println(Base64.getEncoder().encodeToString(secretKeyBytes));
	}

	public String decryption(byte[] message, byte[] hmac) throws NoSuchAlgorithmException, InvalidKeyException {
		// Generate HMAC with ciphertext and secretKey
		Mac HMAC = Mac.getInstance("HmacSHA256");
		SecretKeySpec secretKeySpec = new SecretKeySpec(secretKey.getEncoded(), "HmacSHA256");
		// System.out.println("HMAC : " + HMAC);

		HMAC.init(secretKeySpec);
		byte[] hmacCiphertextBytes = HMAC.doFinal(message);
		// System.out.println("HMAC in bytes : " + hmacCiphertextBytes);
		
		// Compare new HMAC with received HMAC
		if (!Arrays.equals(hmac, hmacCiphertextBytes)) {
			System.out.println("HMAC is not valid, skipping...");
			return "";
			...
			try {
				...
				System.out.println("Decryption didn't work!");
				e.printStackTrace();
				return "";
			}
		}
	...
	public void init() throws IOException {
		...
		try {
			...
			plugin.getLogger().warning("Failed to send keys!");
			e.printStackTrace();
		}
	...
	protected void handleLine(String line) {
		...
		// System.out.println("Recieved HMAC (B64):" + line);
		...
		try {
			...
			// Split the message into MAC and ciphertext
			String base64HMAC = line.substring(0, 44);
			String base64CipherText = line.substring(44);
			// System.out.println("HMAC: " + base64HMAC + "\nCiphertext: " + base64CipherText);

			// Convert base64 message to bytearray
			byte[] byteArrayHMAC = Base64.getDecoder().decode(base64HMAC);
			byte[] byteArrayCipherText = Base64.getDecoder().decode(base64CipherText);

			try {line = decryption(byteArrayCipherText, byteArrayHMAC);} catch (NoSuchAlgorithmException | InvalidKeyException n) {line = "";}
			...
			if (line == "") {
				...
				// Remove random ")" that appears
				String lastElement = args[args.length - 1];
				String cleanedLastElement = lastElement.substring(0, lastElement.length() - 1);
				args[args.length - 1] = cleanedLastElement;
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			switch (c) {
				...
				case "": {
					plugin.getLogger().warning("Client message is not valid");
				}
				default:
